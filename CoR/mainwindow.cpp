...
MainWindow::MainWindow(QWidget *parent, PlotData plData) :
    QMainWindow(parent),
...
{
    winName = "Rotation Center Estimation";
    ...
    QVector<double> y1(g_X.size());
    for (int i=0; i < g_X.size(); ++i)
    {
        y1[i] = m_plData.maxYVal;
    }

    QVector<double> y3(2);
    y3[0] = 0.0;
    y3[1] = m_plData.maxYVal + m_plData.maxYVal/100.0;
    QVector<double> x3(2);
    x3[0] = m_plData.maxXVal;
    x3[1] = m_plData.maxXVal;
    ...
    //-ju- add title layout element:
    customPlot->plotLayout()->insertRow(0);
    QCPPlotTitle *title = new QCPPlotTitle(customPlot, "Shift of rotation center in pixel. Measured as offset from midth of projection image.");
    title->setFont(QFont(font().family(), 10));
    customPlot->plotLayout()->addElement(0, 0, title);
    // make left and bottom axes always transfer their ranges to right and top axes:
    connect(customPlot->xAxis, SIGNAL(rangeChanged(QCPRange)), customPlot->xAxis2, SLOT(setRange(QCPRange)));
    connect(customPlot->yAxis, SIGNAL(rangeChanged(QCPRange)), customPlot->yAxis2, SLOT(setRange(QCPRange)));
    // pass data points to graphs:
    customPlot->graph(0)->setData(g_X, g_Y);
    customPlot->graph(1)->setData(g_X, y1);
    //-ju- max.
    customPlot->graph(2)->setData(x3,y3);
    //-ju- give the axes some labels:
    customPlot->yAxis->setLabel("edge sharpness");
    customPlot->xAxis->setLabel("pixel shift");
    ...
    QString showRot;
    showRot.sprintf(" pixel shift = %7.4f ",m_plData.maxXVal);
    textLabel->setText(showRot);
    textLabel->setFont(QFont(font().family(), 8));
    textLabel->setPen(QPen(Qt::green));

    // add the arrow:
    QCPItemLine *arrow = new QCPItemLine(customPlot);
    customPlot->addItem(arrow);
    arrow->start->setParentAnchor(textLabel->left);
    arrow->end->setCoords(m_plData.maxXVal, m_plData.maxYVal);
    arrow->setHead(QCPLineEnding::esDisc);
}

void MainWindow::setupSincScatterDemo(QCustomPlot *customPlot)
{
    winName = "Sinc Scatter Demo";
...
{
    winName = "Parametric Curves Demo";
...
#endif
    QString fileName = winName.toLower()+".png";
    fileName.replace(" ", "");

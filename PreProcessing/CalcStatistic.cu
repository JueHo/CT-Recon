...
// need linear data --> using flat 
void CalculateSkew(float *data, size_t sizeData, float &skewness)
{
	// transfer to device
	thrust::device_vector<float> d_x(data, data + sizeData);
	// setup arguments
	...
	summary_stats_data<float> result = thrust::transform_reduce(d_x.begin(), d_x.end(), unary_op, init, binary_op);

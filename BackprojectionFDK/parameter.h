...
namespace FDK {
	struct FDK_Parameters
	{
		...
		int highAcc;				// high accurate bi-linear interpolation
		...
		int	viewDirection;			// view direction toward: (X-ray source, detector) (0, 1) 
...
public:
	...
	//FDK_Parameters GetParameters(string paramfile)
	//{
	//	ReadParameters(paramfile);
	//	return m_param;
	//}
	void ReadParameters();
	//void ReadParameters(string paramfile);
	void WriteParameters(FDK_Parameters );
private:
	bool CheckForWhiteSpace(string name);
